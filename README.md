<div align="center">

# 👋 Hi, I'm Uğur Türker Kebeci

### 🚀 Independent Software Developer | 8+ Years of Coding Experience

---

<img src="https://img.shields.io/badge/Status-Available%20for%20Projects-brightgreen?style=for-the-badge">
<img src="https://img.shields.io/badge/Experience-8%2B%20Years-blue?style=for-the-badge">
<img src="https://img.shields.io/badge/Focus-Clean%20Architecture-purple?style=for-the-badge">

</div>

---

## 🧠 About Me

I have been developing software for over 8 years. I see software not just as writing code, but as an **art of system design and problem solving**.  
As an independent developer, I focus on building scalable, clean, and maintainable systems.

---

## 🛠 Tech Stack

| Area | Technologies |
|------|-------------|
| 💻 Programming | Python, PHP |
| 🌐 Web | JavaScript (ES6+), HTML5, CSS3 |
| ⚙️ Systems | Architecture, Optimization, Debugging |
| 🤖 AI / Tools | LLM integrations, automation scripts |

---

## 🚀 Focus Areas

- 🧩 System Architecture & Backend Design
- ⚡ Performance Optimization
- 🧠 AI / LLM-based Applications
- 🔧 Automation Systems
- 🌐 Web Platforms & Dashboards

---

## 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=ugurturkerkebeci&show_icons=true&theme=radical" height="180"/>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=ugurturkerkebeci&layout=compact&theme=radical" height="180"/>
</p>

---

## 🔥 Activity

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=ugurturkerkebeci&theme=shadow-purple&short_numbers=true&mode=weekly"/>
</p>

---

## 🧩 Projects

- 🤖 Local AI LLM systems (GGUF / Ollama / custom runtime)
- ⚙️ Performance analysis tools
- 🧪 Experimental software architectures
- 🌐 Web-based tools and dashboards

---

## 🌐 Contact

<p align="center">
  <a href="https://github.com/ugurturkerkebeci">
    <img src="https://img.shields.io/badge/GitHub-Profile-black?style=for-the-badge&logo=github">
  </a>
  <a href="mailto:turkerkebeci@gmail.com">
    <img src="https://img.shields.io/badge/Email-Contact-red?style=for-the-badge&logo=gmail">
  </a>
</p>

---

<div align="center">

> 💡 "Code is the purest form of thought."

</div>

---

# 🇹🇷 Türkçe Versiyon

## 🧠 Hakkımda

8 yılı aşkın süredir yazılım geliştiriyorum. Yazılımı sadece kod yazmak olarak değil, **sistem tasarımı ve problem çözme sanatı** olarak görüyorum.  
Bağımsız bir geliştirici olarak ölçeklenebilir, temiz ve sürdürülebilir sistemler kurmaya odaklanıyorum.

---

## 🛠 Teknik Yetenekler

| Alan | Teknolojiler |
|------|-------------|
| 💻 Programlama | Python, PHP |
| 🌐 Web | JavaScript (ES6+), HTML5, CSS3 |
| ⚙️ Sistem | Mimari, Optimizasyon, Debugging |
| 🤖 AI / Araçlar | LLM entegrasyonları, otomasyon scriptleri |

---

## 🚀 Odak Alanlarım

- 🧩 Sistem Mimarisi & Backend Tasarımı
- ⚡ Performans Optimizasyonu
- 🧠 AI / LLM tabanlı uygulamalar
- 🔧 Otomasyon sistemleri
- 🌐 Web platformları ve dashboard sistemleri

---

## 🧩 Projelerim

- 🤖 Yerel AI LLM sistemleri (GGUF / Ollama / custom runtime)
- ⚙️ Performans analiz araçları
- 🧪 Deneysel yazılım mimarileri
- 🌐 Web tabanlı araçlar ve paneller

---

## 🌐 İletişim

#

<p align="center">
  <a href="https://github.com/ugurturkerkebeci">
    <img src="https://img.shields.io/badge/GitHub-Profile-black?style=for-the-badge&logo=github">
  </a>

  <a href="mailto:turkerkebeci@gmail.com">
    <img src="https://img.shields.io/badge/Email-Contact-red?style=for-the-badge&logo=gmail">
  </a>
</p>

<div align="center">

> 💡 "Kod, düşüncenin en saf halidir."

</div>
